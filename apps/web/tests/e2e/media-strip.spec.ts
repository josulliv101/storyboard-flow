...
test.describe('MediaStrip E2E Reordering & Scroll Gestures', () => {
  ...
  test('pragmatic adapter pointer drag within strip', async ({ page }) => {
    await page.goto(storyPath('ui-mediastrip-mediastrip--reorderable-media-strips-pragmatic-dnd'));
  ...
  test('nested collection invalid cycle prevention', async ({ page }) => {
    await page.goto(storyPath('ui-mediastrip-mediastrip--deeply-nested-collections'));
  ...
  test('pragmatic adapter nested collection invalid cycle prevention', async ({ page }) => {
    await page.goto(storyPath('ui-mediastrip-mediastrip--deeply-nested-collections-pragmatic-dnd'));
  ...
  test('nested collection drag-and-drop between cards', async ({ page }) => {
    ...
    const handleImg3 = page.locator('[data-reorder-handle="img-3"]');
    const itemImg3 = page.locator('[data-value="img-3"]');
    const cardColB = page.locator('[data-value="card-col-b"]');
    const cardColC = page.locator('[data-value="card-col-c"]');

    await handleImg3.waitFor({ state: 'visible' });
    await itemImg3.waitFor({ state: 'visible' });
    await cardColB.waitFor({ state: 'visible' });
    await cardColC.waitFor({ state: 'visible' });

    const handleBox = await handleImg3.boundingBox();
    expect(handleBox).not.toBeNull();
    ...
    const liveBoxColC = await cardColC.boundingBox();
    expect(liveBoxColC).not.toBeNull();

    // Drag to the live left edge of card-col-c. The center is reserved for
    // nesting, while the edge resolves to an insert before col-c.
    const gapX = liveBoxColC!.x + liveBoxColC!.width * 0.05;
    const gapY = liveBoxColC!.y + liveBoxColC!.height / 2;

    await page.mouse.move(gapX, gapY, { steps: 10 });
    await page.mouse.up();

    // Verify img-3 is now positioned between card-col-b and card-col-c.
    await expect(async () => {
      const bPos = await cardColB.boundingBox();
      const imgPos = await itemImg3.boundingBox();
  ...
  test('virtualized long strip pointer drag persists reorder', async ({ page }) => {
    await page.goto(storyPath('ui-mediastrip-mediastrip--thousands-of-items-virtualized'));
    ...
    const handleBox = await handle20.boundingBox();
    const item21Box = await item21.boundingBox();

    expect(handleBox).not.toBeNull();
    expect(item21Box).not.toBeNull();

    const startX = handleBox!.x + handleBox!.width / 2;
    const startY = handleBox!.y + handleBox!.height / 2;
    const endX = item21Box!.x + item21Box!.width * 0.8;
    const endY = item21Box!.y + item21Box!.height / 2;

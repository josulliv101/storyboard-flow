...
type SidebarClipType = "collection" | "image" | "video";

export type TimelineDropHandlers = {
  ...
  onDropSidebarClip?: (
    insertIndex: number,
    type: SidebarClipType,
  ) => void;
  onDropClipIntoCollection?: (
    clip: TimelineClip,
    targetCollectionTimelineId: string,
    sourceTimelineId: string,
  ) => void;
...
type UseTimelineDropTargetsOptions = TimelineDropHandlers & {
  contentRef: RefObject<HTMLDivElement | null>;
  getClipLeft: (clip: TimelineClip) => number;
...
export function useTimelineDropTargets({
  contentRef,
  getClipLeft,
  getClipWidth,
  hasClips,
  ...
  const handleDrop = useCallback(
    (e: DragEvent<HTMLDivElement>) => {
      ...
      const sidebarType = e.dataTransfer.getData("application/x-gstudio-type") as
        | SidebarClipType
        | "";
      if (
        sidebarType &&

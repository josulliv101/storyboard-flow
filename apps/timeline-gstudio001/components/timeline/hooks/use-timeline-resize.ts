...
export function useTimelineResize({
  ...
  setTrackTranslateX,
  stopInertia,
}: TimelineInteractionSharedOptions) {
  ...
  const getResizedClips = useCallback((clientX: number) => {
    const state = resizeState.current;
    if (!state.baselineClips) return clips;
  ...
  const compensateLeftResize = useCallback((nextClips: TimelineClip[]) => {
    const state = resizeState.current;
    ...
    const durationDelta =
      nextClips[state.anchorIndex].duration -
      state.baselineClips[state.anchorIndex].duration;
    const targetScrollLeft =
      state.startScrollLeft + durationDelta * safePixelsPerSecond;
  ...
  const handleResizeMove = useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      const state = resizeState.current;
      if (!state.active || !state.baselineClips) return;
      ...
      if (state.edge === "left") compensateLeftResize(nextClips);
      else setTrackTranslateX(0);
    ...
    (event: React.PointerEvent<HTMLDivElement>) => {
      ...
      if (state.edge === "left") {
        ...
        const maxDuration =
          nextClips.reduce(
            (maximum, clip) =>
              Math.max(maximum, clip.startTime + clip.duration),
            0,
          ) + TIMELINE_TRAILING_PADDING_SECONDS;
        const expectedScrollLeft = Math.max(
          0,
          Math.ceil(maxDuration * safePixelsPerSecond) -
  ...
  const handleResizeKeyDown = useCallback(
    (
      ...
      if (edge === "left") {
        if (clip.index !== 0) {
          const durationDelta =
            nextClips[clip.index].duration - clips[clip.index].duration;
          writeScrollLeft(
            Math.max(
              0,

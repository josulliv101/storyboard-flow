import { CLIP_GAP_SECONDS, VIDEO_SOURCES, ITEM_HEIGHT, } from "./constants";
export const MEDIA = [
...
export function getSourceTimeFromClientX({ clientX, rectLeft, rectWidth, sourceDuration, }) {
    if (rectWidth <= 0 || sourceDuration <= 0)
        return 0;
...
export function getTrimHandleSourceTime(clip, edge) {
...
export function getFallbackImage(index, imageWidth) {
    return {
        src: `https://picsum.photos/seed/smooth-scroll-${index}/${imageWidth}/${ITEM_HEIGHT}`,
        alt: `Image ${index}`,
    };
}
export function getSpec(index) {
    return MEDIA[index % MEDIA.length];
}
export function baseWidth(index) {
    return Math.round(ITEM_HEIGHT * getSpec(index).aspect);
}
export function getPackedDurationBefore(clips, anchorIndex) {

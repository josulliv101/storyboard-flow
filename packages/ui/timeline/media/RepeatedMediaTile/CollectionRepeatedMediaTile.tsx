...
const collectionRepeatedMediaTile = cva(
  "group relative h-full w-full select-none overflow-hidden rounded-lg border border-sky-500/20 bg-gradient-to-b from-zinc-900/90 to-zinc-950/95 shadow-xl transition-all duration-300 hover:border-sky-500/40 hover:shadow-sky-950/20",
  {
    variants: {
      density: {
        compact: "p-2.5",
...
export function CollectionRepeatedMediaTile({
  ...
  return (
    <div className={collectionRepeatedMediaTile({ density })}>
      ...
      {!isXS && (
        <div className="absolute inset-0 flex items-center px-3.5 py-3.5">
          <div className="grid h-24 max-h-full w-full grid-cols-3 gap-1.5 rounded-lg border border-zinc-900/80 bg-zinc-950/70 p-1.5 shadow-inner">
            {([
              { endpoint: "first" as const, item: previewItems[0] },
              { endpoint: null, item: null },
              {
                endpoint: "last" as const,
              ...
              if (endpoint === null) {
                const countContent = (
                  <>
                    <span className="text-lg font-black leading-none text-sky-100">
                      {clip.itemCount}
                    </span>
                    <span className="mt-0.5 text-[9px] font-bold uppercase tracking-wider text-sky-400/85">
                      {clip.itemCount === 1 ? "item" : "items"}
                    </span>
                  </>
                );
                ...
                if (collectionHref) {
                  return (
                    <a
                      key={`${clip.id}-item-count`}
                      href={collectionHref}
                      className={collectionItemCountSlot({ density })}
                      aria-label={`Open ${clip.title} timeline (${clip.itemCount} ${
                        clip.itemCount === 1 ? "item" : "items"
                      })`}
                ...
                return (
                  <div
                    key={`${clip.id}-item-count`}
                    className={collectionItemCountSlot({ density })}
                    aria-label={`${clip.title} contains ${clip.itemCount} ${
                      clip.itemCount === 1 ? "item" : "items"
                    }`}
                  >
                    {countContent}

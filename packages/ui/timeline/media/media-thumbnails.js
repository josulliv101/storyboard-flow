...
function getCloudinaryVersionedVideoPath(uploadPath) {
...
export function getThumbnailSlotCount(availableWidth, frameSize) {
    return Math.min(FILMSTRIP_MAX_FRAMES, Math.max(1, Math.ceil(Math.max(1, availableWidth) / Math.max(1, frameSize))));
}
export function getEndpointFrameTimes({ count, endTime, startTime = 0, }) {
...
export function getVideoThumbnailUrl(src, second, width = 480, height = 270) {

...
import { THUMBNAIL_GAP, TIMELINE_TRAILING_PADDING_SECONDS, VISIBLE_OVERSCAN_PX, } from "../constants";
import { getTimelineGridContentWidth, getTimelineGridItemLayout, } from "../timeline-grid";
export function useTimelineLayout({ clips, closingOverhangOffset, firstOverhang, isResizing, lastOverhang, pixelsPerSecond, scrollLeft, scrollTop, gridMetrics, itemTop, thumbnailMode, thumbnailWidth, viewportClientHeight, viewportClientWidth, }) {
    var _a;
    const maxDurationDuringDragRef = useRef(null);
    const baseTotalDuration = useMemo(() => {
        const lastClipEnd = clips.reduce((maximum, clip) => Math.max(maximum, clip.startTime + clip.duration), 0);
        return lastClipEnd + TIMELINE_TRAILING_PADDING_SECONDS;
    }, [clips]);
    let totalDuration = baseTotalDuration;
    if (isResizing) {
        (_a = maxDurationDuringDragRef.current) !== null && _a !== void 0 ? _a : (maxDurationDuringDragRef.current = baseTotalDuration);
    ...
    const visibleClips = useMemo(() => {
        ...
        return clips.filter((clip) => {
            if (thumbnailMode && gridMetrics.enabled) {
                ...
                return (clipEndPx >= visibleStartPx &&
            ...
            return (clip.startTime + clip.duration >= visibleStartTime &&
    ...
    const timelineWidth = Math.max(viewportClientWidth || 1, contentWidth + firstOverhang + lastOverhang);
    return {
        totalDuration,
        visibleClips,
        timelineWidth,

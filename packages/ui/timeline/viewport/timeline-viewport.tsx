...
import {
  ...
  TimelineDropOverlay,
} from "../overlays/timeline-drop-overlays";
...
type CollectionEndpointLinkMarker = {
...
export function TimelineViewport({
  ...
  const collectionEndpointLinkMarkers = useMemo(() => {
    const markers: CollectionEndpointLinkMarker[] = [];
    ...
    for (let index = 0; index < visibleClips.length - 1; index += 1) {
      ...
      const collectionClip =
        leftClip.kind === "collection" ? leftClip :
        rightClip.kind === "collection" ? rightClip :
        null;
      const endpointClip =
        leftClip.viewRole === "collection-endpoint" ? leftClip :
        rightClip.viewRole === "collection-endpoint" ? rightClip :
        null;

      if (!collectionClip || !endpointClip) continue;
      if (!collectionClip.viewExpansionKey) continue;
      if (endpointClip.viewParentCollectionKey !== collectionClip.viewExpansionKey) continue;
      if (!endpointClip.viewEndpoint) continue;

      const leftTop = getClipTop(leftClip);
      const rightTop = getClipTop(rightClip);
      if (Math.abs(leftTop - rightTop) > 1) continue;
            ...
            <>
              ...
              {collectionEndpointLinkMarkers.map((marker) => {
                ...
                <button
                  key={marker.id}
                  type="button"
                  data-testid="timeline-collection-endpoint-link"
                  data-endpoint={marker.endpoint}
                  aria-label={`Hide ${marker.endpoint} collection endpoint`}
                  title={`Hide ${marker.endpoint} endpoint`}
                  className="absolute z-[42] grid h-8 w-8 place-items-center rounded-full border border-sky-300/50 bg-zinc-950/85 text-sky-200 shadow-[0_8px_20px_rgba(2,132,199,0.35)] ring-1 ring-black/40 backdrop-blur transition-colors hover:border-sky-200 hover:bg-sky-950 focus-visible:outline focus-visible:outline-2 focus-visible:outline-amber-300 focus-visible:outline-offset-2"
                  style={{
                    left: `${marker.left}px`,
                    top: `${marker.top}px`,
                  ...
                  onClick={(event) => {
                    ...
                    event.stopPropagation();
                    onToggleCollectionEndpoint?.(marker.collectionClip, marker.endpoint);
                  }}
                >
              ...
              {!interactions.isReordering &&
                ...
                showPlayBarArea &&
                visibleClips.map((clip) =>
                  clip.kind === "video" || clip.kind === "image" || clip.kind === "collection" ? (

...
import type { UploadTimelineMedia } from "./smooth-scroll-list";
import { reindexAndPackClips } from "../hooks/use-timeline-clips";

type TimelineUploadHandlerOptions = {
  applyLocalClipsNow: (clips: TimelineClip[]) => void;
  clips: TimelineClip[];
...
export function useTimelineUploadHandler({
  applyLocalClipsNow,
  clips,
  getTimelineDocument,
  getTimelinePath,
  ...
  const handleDropFiles = useCallback(
    async (insertIndex: number, files: File[]) => {
      ...
      try {
        const newClipResults: Array<{
          ...
          files.map(async (file, index) => {
            ...
            if (thisTimelineId.startsWith("asset-library")) {
              ...
              const pathSegments = getTimelinePath(thisTimelineId).map(
            ...
            try {
              hostedMedia = await uploadTimelineMedia(
                file.name,
                file,
                folderPath,
              );
              ...
              console.warn(
                `Failed to upload "${file.name}" to hosted media storage`,
                error,
              );
              return {
                clip: null,
        ...
        const firstUploadError = newClipResults.find(
          (result) => result.error,
        )?.error;
        if (firstUploadError) {

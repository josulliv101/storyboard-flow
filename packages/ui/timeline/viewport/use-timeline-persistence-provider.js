...
export function useTimelinePersistenceProvider({ applyClipsNow, canPersistTimeline, clips, getTimelineDocument, latestClipsRef, localEditVersionRef, emitTimelineUpdate, persistedTimelineTitle, registerTimelineDocument, setPersistedTimelineTitle, timelineId, timelineTitle, }) {
    const [timelineLoadError, setTimelineLoadError] = useState(null);
    const [isLoadingTimeline, setIsLoadingTimeline] = useState(false);
    const saveBaselineRef = useRef(null);
    const hydratedTimelineIdRef = useRef(null);
    useEffect(() => {
        var _a;
        ...
        if (!timelineId)
            return;
        let isCurrent = true;
        const loadStartedAtLocalEditVersion = (_a = localEditVersionRef.current) !== null && _a !== void 0 ? _a : 0;
        const loadTimelineDocument = async () => {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    if (isCurrent) {
                        setTimelineLoadError(result.error ||
                ...
                if (result.document.id !== timelineId) {
                    setTimelineLoadError(`Timeline response returned "${result.document.id}" instead of "${timelineId}".`);
                    return;
                }
                ...
                if (localEditVersionRef.current !== loadStartedAtLocalEditVersion) {
                    ...
                    const shouldKeepLocalClips = latestClips.length > 0 || result.document.clips.length === 0;
                    const resolvedDocument = shouldKeepLocalClips
                        ? Object.assign(Object.assign({}, result.document), { clips: latestClips }) : result.document;
                    registerTimelineDocument(resolvedDocument);
                    emitTimelineUpdate(resolvedDocument.id, resolvedDocument);
                    if (!shouldKeepLocalClips) {
                        const loadedClips = result.document.clips.map((clip) => (Object.assign({}, clip)));
                ...
                if (isCurrent) {
                    setTimelineLoadError(error instanceof Error
                        ? error.message
    ...
    useEffect(() => {
        ...
        if (document) {
            if (clips.length === 0 && document.clips.length > 0)
                return;
            registerTimelineDocument(Object.assign(Object.assign({}, document), { clips }), { persist: false });
        }
    }, [
        canPersistTimeline,
        clips,
    ...
    useEffect(() => {
        ...
        const document = getTimelineDocument(thisTimelineId);
        if (clips.length === 0 && (document === null || document === void 0 ? void 0 : document.clips.length))
            return;
        ...
        if (isUnsavedProjectPlaceholder(documentSnapshot))
            return;
        const serialized = JSON.stringify(documentSnapshot);
        if (serialized === saveBaselineRef.current)
            return;

...
import {
  DragPreviewPortal,
  type DragPreviewCoordinates,
} from "./DragPreviewPortal";

function PreviewCard({ label = "Dragged item" }: { label?: string }) {
  return (
    <div className="flex h-full w-full items-center justify-center rounded-md border border-sky-300/50 bg-sky-950 px-3 text-sm font-semibold text-sky-50 shadow-2xl shadow-black/40">
      {label}
    </div>
  );
}
...
function LifecycleDragDropDemo() {
  const [preview, setPreview] = React.useState<DragPreviewCoordinates | null>(
    null,
  );
  ...
  function finishDrag(event: React.PointerEvent<HTMLButtonElement>) {
    const dropBounds = dropTargetRef.current?.getBoundingClientRect();
    const landedInTarget = Boolean(
      dropBounds &&
  ...
  return (
    <div className="relative h-[260px] w-full">
      <button
        ...
        data-dragging={preview !== null}
        className="absolute left-8 top-8 flex h-[72px] w-40 touch-none select-none items-center justify-center rounded-md border border-sky-300/50 bg-sky-950 px-3 text-sm font-semibold text-sky-50 shadow-lg shadow-black/30 data-[dragging=true]:opacity-30"
      ...
      <div
        ...
        className="absolute right-8 top-24 flex h-28 w-52 items-center justify-center rounded-md border border-dashed border-zinc-600 bg-zinc-900/80 text-sm font-semibold text-zinc-300 data-[dropped=true]:border-emerald-300 data-[dropped=true]:bg-emerald-950 data-[dropped=true]:text-emerald-100"
      >
        {isDropped ? "Dropped" : "Drop target"}
      </div>
      ...
      <DragPreviewPortal
        ...
        width={160}
        height={72}
        testId="lifecycle-drag-preview"
      >
        <PreviewCard label="Clip A" />
      </DragPreviewPortal>
    </div>
  );
}

const meta: Meta<typeof DragPreviewPortal> = {
  title: "UI/Drag Drop/DragPreviewPortal",
  component: DragPreviewPortal,
  decorators: [
    (Story) => (
      <div
        className="relative min-h-[260px] overflow-hidden rounded-lg border border-zinc-800 bg-zinc-950 text-zinc-100"
        style={{
          backgroundImage:
      ...
      >
        <div className="absolute left-8 top-8 h-16 w-32 rounded-md border border-zinc-700 bg-zinc-900/80" />
        <Story />
      </div>
    ),
  ],
...
type Story = StoryObj<typeof DragPreviewPortal>;

export const HiddenWithoutPreview: Story = {
  args: {
    preview: null,
...
export const CustomScaleAndClass: Story = {
  args: {
    preview: {
      clientX: 280,
      clientY: 170,
    ...
    scale: 1.12,
    className:
      "fixed pointer-events-none z-[9999] rounded-lg ring-2 ring-emerald-300",
    testId: "drag-preview-custom",
    children: <PreviewCard label="Custom ghost" />,
  },
  play: async () => {
    const body = within(document.body);
...
export const PickUpDragAndDrop: Story = {
  render: () => <LifecycleDragDropDemo />,

...
export function GalleryCanvasPreview({ snapshot, isPlaying = false, }) {
    const canvasRef = React.useRef(null);
    const videoRef = React.useRef(null);
    const drawableRef = React.useRef(null);
    ...
    const videoFrameHandleRef = React.useRef(null);
    const animationFrameHandleRef = React.useRef(null);
    ...
    const stopFrameLoop = React.useCallback(() => {
        ...
        if (videoFrameHandleRef.current !== null && (video === null || video === void 0 ? void 0 : video.cancelVideoFrameCallback)) {
    ...
    const startFrameLoop = React.useCallback(() => {
        ...
        if (video.requestVideoFrameCallback) {
            const drawVideoFrame = () => {
                var _a, _b;
                if (mediaKeyRef.current !== frameMediaKey || video.currentSrc !== frameSource)
                    return;
                drawCurrentFrame();
                if (!video.paused && !video.ended) {
                    videoFrameHandleRef.current = (_b = (_a = video.requestVideoFrameCallback) === null || _a === void 0 ? void 0 : _a.call(video, drawVideoFrame)) !== null && _b !== void 0 ? _b : null;
                }
            };
    ...
    const seekToLatestTarget = React.useCallback((video) => {
    ...
    React.useEffect(() => {
        ...
        const isCurrentSource = () => (expectedVideoSourceRef.current !== '' &&
    ...
    React.useLayoutEffect(() => {
        ...
        if (targetImage.complete && targetImage.naturalWidth > 0)
            ...
            targetImage.addEventListener('load', drawImage, { once: true });
    ...
    React.useEffect(() => {
        ...
        if (!video || snapshot.media.type !== 'video')
            return;
        video.muted = !isPlaying;
    ...
    return (_jsx("canvas", { ref: canvasRef, role: "img", "aria-label": `${snapshot.media.name} scrub preview`, className: "block h-full w-full rounded-md bg-black" }));
}

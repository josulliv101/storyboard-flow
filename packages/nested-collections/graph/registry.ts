// Graph — the kind -> node type map.
//
// One function, and it is the only one in the core that throws. That is why it
 ...
 *
 * THROWS on a duplicate kind, and this is the only function in the core that
...
export function buildRegistry(types: readonly WidenedNodeType[]): NodeTypeRegistry {
  ...
  for (const nodeType of types) {
    if (registry.has(nodeType.kind)) {
      throw new Error(
        `nested-collections: duplicate node kind ${JSON.stringify(nodeType.kind)} in ` +

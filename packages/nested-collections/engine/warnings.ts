...
const warnIfCacheCannotCover = (candidate: SizedGraph): void => {
  ...
  console.error(
    `nested-collections: this graph holds ${nodeCount} nodes and ${foldCount} fold(s) are registered, ` +
...
const warnIfCommitCostIsPastInteractive = (candidate: SizedGraph): void => {
  ...
  console.error(
    `nested-collections: this graph holds ${nodeCount} live nodes, past the ${budget} this engine ` +

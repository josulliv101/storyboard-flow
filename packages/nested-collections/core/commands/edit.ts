...
export function planEdits<Ts extends readonly WidenedNodeType[], S>(
  ...
  for (const edit of edits) {
    ...
    if (ctx.devChecks) {
      ...
      if (verdict === false) {
        console.error(
          `nested-collections dev check: editing a ${JSON.stringify(node.kind)} node stored something ` +
      ...
      if (invert !== undefined) {
        try {
          ...
          if (!back.ok) {
            console.error(
              `nested-collections dev check: ${JSON.stringify(node.kind)}.invertEdit produced an edit its ` +
                `own applyEdit refuses. node=${JSON.stringify(edit.nodeId)}`,
            );
          } else if (structurallyEqualBounded(back.value, node.data) === false) {
            console.error(
              `nested-collections dev check: ${JSON.stringify(node.kind)}.invertEdit does not undo its edit. ` +
          ...
          console.error(
            `nested-collections dev check: ${JSON.stringify(node.kind)}.invertEdit threw. ` +

...
//   2. the store, which is the only stateful thing in the core.
...
>(
  ...
  for (const [entryKey, fold] of Object.entries(config.folds)) {
    ...
    if (prior !== undefined) {
      throw new Error(
        `nested-collections: duplicate fold key ${JSON.stringify(fold.key)} — registered as ` +
  ...
  for (const [name, value] of [
    ...
    if (!Number.isInteger(value) || value <= 0) {
      throw new Error(
        `nested-collections: ${name} must be a positive integer, received ${String(value)}. ` +
  ...
  const compareShadow = (
    ...
    try {
      ...
      console.error(
        `nested-collections dev check: a shadow cold refold of ${JSON.stringify(key)} threw. ` +
    ...
    console.error(
      `nested-collections dev check: the memo table served a STALE ${JSON.stringify(key)} for node ` +
  ...
  const shadowCheck = (
    ...
    if (shadowsLeft === 0) {
      console.error(
        `nested-collections dev check: the shadow cold refold has spent its budget of ` +
  ...
  const createStore = (initialGraph: Graph<Ts, S>): Store<Ts, S, F> => {
    ...
    const notifyOne = (label: string, deliver: () => void): void => {
      try {
        ...
        console.error(
          `nested-collections: a ${label} subscriber threw. The commit is unaffected and the ` +
    ...
    const auditIfDev = (label: string, next: Graph<Ts, S>): void => {
      ...
      console.error(
        `nested-collections: invariant violated after ${label}: ${violation.code} — ${violation.message}`,
        violation,
      );
    };

...
//
// THIS FILE IS THE `.` ENTRY, and the `exports` map is load-bearing: it must
// never gain a re-export of anything under `react/`. A single barrel covering
// both halves would put `"use client"` back in a server bundle's path, which is
// this repo's most expensive bug class and one CI is blind to.
//

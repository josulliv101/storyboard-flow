...
export function serializeGraph<Ts extends readonly WidenedNodeType[], S>(
  ...
  const serializeData = (kind: string, data: unknown): unknown => {
    ...
    try {
      ...
      console.error(
        `nested-collections: ${quoteFromWire(kind)}.serialize threw while writing a node for save. ` +
    ...
    if (node.container) {
      ...
      if (node.summary !== null) {
        ...
        try {
          ...
          console.error(
            `nested-collections: the summary type's serialize threw while writing node ${quoteFromWire(id)} for save. ` +

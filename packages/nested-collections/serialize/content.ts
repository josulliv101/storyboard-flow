...
export function runContentDevChecks(
  ...
  try {
    ...
    console.error(
      `nested-collections dev check: ${what} threw while serializing during the round-trip audit. ` +
  ...
  try {
    ...
    console.error(
      `nested-collections dev check: ${what} threw while re-parsing its own serialize output. ` +
        describeThrown(thrown),
    );
    return;
  }
  if (!again.ok) {
    console.error(
      `nested-collections dev check: ${what} produced serialize output its own parse refuses. ` +
  ...
  console.error(
    `nested-collections dev check: ${what} did not survive a parse(serialize(d)) round trip. ` +

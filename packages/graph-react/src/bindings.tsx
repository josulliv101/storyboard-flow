...
//
// It also has to be a SEPARATE PACKAGE from graph-core, not just a separate
 ...
 *
 * STILL "ERASED" while graph-core's equivalent is now `WidenedNodeType`, and the
 ...
 * graph-core's boundary constructors make, one level up.
 *
...
>(engine: Engine<Ts, S, F>): ReactBindings<Ts, S, F> {
  ...
  const Provider: FunctionComponent<ProviderProps<Ts, S, F>> = ({
    ...
    // nodes to these views. graph-core covers the mutating paths with an
  ...
  function useStore(): BoundStore {
    ...
    if (store === null) {
      ...
      // graph-core: every `Rejection` there names something a consumer can act on
   ...
   * looks like the more precise choice and is not. graph-core documents a
  ...
  // that prunes the selection MUST notify selection ones. graph-core owns both
  ...
  const selectSelectedIds = (selection: SelectionSlice): readonly NodeId[] =>
    // Identity-stable: graph-core only reassigns the array when the set actually
  ...
  return {
    ...
    useChildren,
    useFold,
    // Same function, both names. graph-core sets the precedent with

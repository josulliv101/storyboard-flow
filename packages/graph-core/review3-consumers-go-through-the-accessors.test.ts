...
// graph-core at the time, and two of them were reading `childrenById` raw precisely
...
function scan(): Readonly<{ checked: readonly string[]; violations: readonly Violation[] }> {
  ...
  // `packages/graph-core` is the implementation: it MUST read its own maps.
  const engineDir = join(root, "packages", "graph-core") + sep;
  ...
  for (const file of files) {
    ...
    if (!text.includes("@storyboard/graph-core") && !text.includes("@storyboard/graph-react")) {
      continue;
    }
...
describe("Graph consumers reach the graph through its accessors", () => {
  it("finds the consumer files at all", () => {
    ...
    expect(checked.some((f) => f.startsWith("packages/graph-react/"))).toBe(true);
  });

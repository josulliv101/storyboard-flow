...
//
// One function, and it is the only one in graph-core that throws. That is why it
 ...
 *
 * THROWS on a duplicate kind, and this is the only function in graph-core that
...
export function buildRegistry(types: readonly WidenedNodeType[]): NodeTypeRegistry {
  ...
  for (const nodeType of types) {
    if (registry.has(nodeType.kind)) {
      throw new Error(
        `graph-core: duplicate node kind ${JSON.stringify(nodeType.kind)} in ` +
